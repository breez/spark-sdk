...
| No | Yes | ⏸️  STOP - Don't duplicate. Use `.claude/skills/pr-review/add-review-comment.sh` if you found new issues |
...
Use efficient diff-based review (see `.claude/skills/pr-review/diff-workflow.md`):
```bash
# Get filtered diff
./.claude/skills/pr-review/filter-diff.sh <pr-number>
...
./.claude/skills/pr-review/add-review-comment.sh \
  <pr-number> \
  COMMENT \
...
# 4. Post
./.claude/skills/pr-review/post-review.sh 569 REQUEST_CHANGES "Summary" comments.json
```
