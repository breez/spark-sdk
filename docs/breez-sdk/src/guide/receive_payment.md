...
| **PaymentUpdated**   | The cross-chain provider has marked the order terminal and the conversion details are now attached. Fires after **PaymentSucceeded** for cross-chain receives, since the Spark transfer can settle before the provider closes the order. | Re-read the payment to surface the conversion details (source asset, delivered amount, provider fees). |
