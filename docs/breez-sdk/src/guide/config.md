...
The recommended way to opt into that profile is via {{#name default_server_config}}, which returns the same `Config` as {{#name default_config}} with this flag flipped off. See [Serving end-user wallets](./server_mode.md) for the full profile, lifecycle pattern, and shared-infrastructure wiring. A single long-lived wallet on a server does **not** want this flag: see [Treasury wallets](./treasury.md). Configuring this field directly is supported if you build your `Config` another way:
...
Set it to `false` when collecting behind every operation costs more than it is worth, on a busy wallet holding many leaves, or where bandwidth matters more than being able to recover funds without the operators. See [Treasury wallets](./treasury.md) for when that trade is worth making.

Only the automatic collection is governed. {{#name sync_wallet}} collects regardless of this flag, and waits for the pass before returning rather than leaving it to the background, so **calling a sync yourself is how the data is kept current with the automatic collection off**. A leaf whose chain the operators will not complete stays un-exitable until a later attempt succeeds, so a successful pass means the collection ran, not that every leaf is now exitable.
