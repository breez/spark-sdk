...
- **Sending:** No proof of payment: a Spark transfer produces no Lightning preimage, so an LNURL success action encrypted with the preimage cannot be decrypted. The payment is otherwise reported as the Bolt11 invoice it paid, keeping its description and any LNURL metadata.
