...
The SDK supports various configuration options to customize its behavior. During [initialization](./initializing.md#basic-initialization), you must provide a configuration object, which we recommend creating by modifying the default configuration.
...
Some configuration options can be updated while the SDK is still running. Use the {{#name updateConfig}} method to apply partial updates — only fields that are set will be modified, while fields not set remain unchanged.

The following fields can be updated:

- **Synchronization interval** — change how frequently background syncs occur
- **Max deposit claim fee** — adjust or disable automatic deposit claiming
- **Prefer Spark over Lightning** — toggle settlement preference
- **Stable balance configuration** — enable, update, or disable stable balance auto-conversion

For stable balance configuration, use {{#enum StableBalanceConfigUpdate::Set}} to enable or update, and {{#enum StableBalanceConfigUpdate::Unset}} to disable auto-conversion.

For max deposit claim fee, use {{#enum MaxDepositClaimFeeUpdate::Set}} to set an explicit fee limit, and {{#enum MaxDepositClaimFeeUpdate::Unset}} to disable automatic claiming.

{{#tabs config:update-config}}
