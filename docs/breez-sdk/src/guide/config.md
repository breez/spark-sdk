...
By default a deposit is only claimed once it reaches enough confirmations to mature. Setting {{#name max_instant_deposit_claim_fee_bps}} opts into instant claims: the Spark Service Provider fronts the credited amount ahead of maturity and takes a spread, and this value caps that spread in basis points of the deposit value (100 bps = 1%).
...
More information can be found in the [Claiming on-chain deposits](./onchain_claims.md#instant-claims) page.
