...
When the SDK is built for [serving end-user wallets](server_mode.md), {{#name get_info}} reads the balance live from the spark wallet's local tree store rather than from the background-maintained cache. As a result:
