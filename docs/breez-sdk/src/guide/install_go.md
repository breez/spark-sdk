...
For [iOS](#ios) the native binary framework needs to be additionally installed using [Swift Package Manager](#swift-package-manager).
...
When targeting iOS, you must also install the native binary framework. This is the same framework used by the [Swift Breez SDK package](install_ios_swift.md) and can be installed via [Swift Package Manager](#swift-package-manager).

**Note:** The Go and Swift packages **MUST** have the same version. A version mismatch between the two will cause linking or runtime errors.
