...
| **PaymentUpdated**   | The conversion details on an already-settled payment changed (e.g. the provider order transitioned from `Pending` to `Completed` after the Spark transfer had already settled). | Re-read the payment to surface the updated conversion details (delivered amount, provider fees, final status). |
