...
- **[Server deployments](server_deployments.md)** covers the two shapes the SDK takes on a server: [serving end-user wallets](server_mode.md), where each request builds an ephemeral SDK and the host orchestrates sync, claiming, and event delivery explicitly, and [treasury wallets](treasury.md), a single long-lived wallet your own service owns
