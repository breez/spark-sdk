...
- **[Server-side configuration](server_deployments.md)** covers the two ways the SDK runs on a server: [multi-user](server_mode.md), where each request builds a short-lived SDK and the host drives sync, claiming and event delivery, and [treasury](treasury.md), a single long-lived wallet your own service owns
