...
Explicit operations such as {{#name sync_wallet}}, {{#name claim_deposit}}, {{#name list_unclaimed_deposits}}, {{#name refund_deposit}}, and {{#name refund_pending_conversions}} continue to work and are the intended entry points in this configuration.
...
The {{#enum SdkEvent::Synced}} event pattern documented in [Listening to events](events.md) is **not available** in this configuration: the SDK has no background subscriber to emit it. Treat {{#name sync_wallet}} as the synchronous primitive instead.
...
If [{{#name stable_balance_config}}](./config.md#stable-balance-configuration) is set in this configuration, SDK initialization fails with an invalid input error. Explicit token conversion flows used by payment APIs can still be used, but do not configure Stable Balance in this configuration.
