...
On `FeesExcluded` sends and receives the SDK pads the user's target amount by `target_overpay_bps` so the realized delivery lands at or above target despite provider slippage.
...
Values outside 0 to 500 are rejected at both config validation and per-request validation. `FeesIncluded` requests ignore this field.
