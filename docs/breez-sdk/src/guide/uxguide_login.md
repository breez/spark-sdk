...
2. **One button when the platform allows it.** On iOS and Android, a single primary action ({{#name PasskeyClient.connect_with_passkey}}) serves both new and returning users: it signs a returning user in silently and falls through to registration for a new one. On web, do the same where {{#name PasskeyClient.supports_immediate_mediation}} is true; otherwise split into "Create a new passkey" and "Sign in with a passkey", because without immediate mediation WebAuthn can't distinguish "no credential" from "user cancelled".
