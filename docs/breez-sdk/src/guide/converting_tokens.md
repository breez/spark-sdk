...
This process enables funds to be converted either to or from Bitcoin and a selected token. The conversion process takes two steps:

1. [Preparing to convert](#preparing-to-convert)
1. [Convert](#convert)
...
The first step is to prepare the conversion. This step validates that a conversion between Bitcoin and the selected token is possible, that the amount is within the limits of the conversion, confirms that sufficient funds are available, and provides estimates for the received amount and associated fees.
