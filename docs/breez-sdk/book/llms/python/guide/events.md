...
| `SdkEvent.PAYMENT_METADATA_UPDATED` | `Payment` | Details of a payment already reported changed, for example the conversion info of a cross-chain receive that arrived after the payment settled. |
| `SdkEvent.NEW_DEPOSITS` | `DepositInfo` list | On-chain deposits were detected, possibly while still unconfirmed. Show them as pending until claimed: some may be credited early through an [instant or expedited claim](onchain_claims.md#instant-expedited-claims). |
...
| `SdkEvent.UNILATERAL_EXIT_STATE_CHANGED` | none | An exit state exported earlier is now out of date. Export it again. See [unilateral exit](unilateral_exit.md). |
...
| `is_mature` | Whether the deposit has reached the standard claim depth (3 confirmations). Deposits below it can still be credited through an [instant or expedited claim](onchain_claims.md#instant-expedited-claims). |
