...
| `proxy` with `connections_per_operator` above 1 | Balanced operator connections build their own connectors and cannot be routed. |
...
- `get_spark_status`, via `GetSparkStatusRequest`
- `new_rest_chain_service`, via `NewRestChainServiceRequest`
