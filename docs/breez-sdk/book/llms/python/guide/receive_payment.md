...
1. **Detected**: The SDK detects the deposit, often while it is still in the mempool, and emits a `SdkEvent.NEW_DEPOSITS` event.
2. **Claimed automatically**: After **3 on-chain confirmations** the SDK attempts a standard claim. With a high enough [max claim fee](/llms/python/guide/config.md#max-deposit-claim-fee) it can credit the deposit sooner, even before it confirms, through an [instant or expedited claim](/llms/python/guide/onchain_claims.md#instant-expedited-claims).
...
To track deposits that have not reached the standard claim depth yet, use `list_unclaimed_deposits` and filter by the `is_mature` field:
...
The `amount` on `ReceivePaymentMethod.CROSS_CHAIN` is in the source asset's base units, per the route's `CrossChainRoutePair.decimals`. USD-stable sources sit at USD parity, so `1_000_000` is 1 USDC (6 decimals), about $1.
...
The provider's fee in `service_fee_amount` is in its own asset, `service_fee_asset`, which can differ from both the deposit and the destination asset. Format it with `service_fee_asset_decimals`, not the route's decimals.
...
| Event                 | Description                                                                                                                              | UX Suggestion                                                                                               |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| **NewDeposits**       | New deposits were detected. Each deposit includes a `is_mature` field indicating whether it has reached the standard claim depth. | Show the deposit to the user as pending until it is claimed.          |
...
| **PaymentMetadataUpdated** | The provider confirmed the order and the payment now carries its `conversion_info`. This can arrive after **PaymentSucceeded**, which may report the payment as a plain Spark transfer. | Refresh the payment in the UI with the details carried by the event. |
