...
| **PaymentMetadataUpdated** | The provider confirmed the order: the payment's `conversion_info` now carries the delivered amount and the deposit transaction. The payment is reported with its `conversion_info` from its first event on, so this only completes it. | Refresh the payment in the UI with the details carried by the event. |
