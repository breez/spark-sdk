...
4. **Give every user a human-readable Lightning address.** Register a random one automatically so receiving works from the first moment, and let the user customize it later. If they change it, tell them the old address stops receiving payments. It stays reserved for them, so no one else can take it and they can switch back later.
...
8. **Reflect payment progress as it happens**, driven by SDK events: see the [receive payment UX recommendations](/llms/react-native/guide/receive_payment.md#lightning-1).
