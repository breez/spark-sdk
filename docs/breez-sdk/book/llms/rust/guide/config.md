...
API docs: https://breez.github.io/spark-sdk/breez_sdk_spark/struct.ProxyConfig.html
...
USDC/USDT (send and receive) requires explicit opt-in: `default_config` leaves `cross_chain_config` unset. Set it to a default `CrossChainConfig` to enable the feature, or to your own to override the slippage default. The SDK only returns routes whose destination is USDC or USDT on a supported chain.
