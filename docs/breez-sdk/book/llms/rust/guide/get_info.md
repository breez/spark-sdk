...
2. Subscribe to events and call `get_info` again on each `SdkEvent::Synced` event to fetch the latest balance. See [Listening to events](/llms/rust/guide/events.md).

| Event | Description | UX Suggestion |
| ----- | ----------- | ------------- |
| `SdkEvent::Synced` | The SDK has synced with the network in the background. | Call `get_info` to refresh the displayed balance, and refresh the payments list. See [listing payments](/llms/rust/guide/list_payments.md). |
...
When the SDK is built with the [multi-user configuration](server_mode.md), `get_info` reads the balance live from the spark wallet's local tree store rather than from the background-maintained cache. As a result:
