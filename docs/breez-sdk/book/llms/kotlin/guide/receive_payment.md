...
1. **Detected**: The SDK detects the deposit, often while it is still in the mempool, and emits a `SdkEvent.NewDeposits` event.
2. **Claimed automatically**: After **3 on-chain confirmations** the SDK attempts a standard claim. With a high enough [max claim fee](/llms/kotlin/guide/config.md#max-deposit-claim-fee) it can credit the deposit sooner, even before it confirms, through an [instant or expedited claim](/llms/kotlin/guide/onchain_claims.md#instant-expedited-claims).
...
To track deposits that have not reached the standard claim depth yet, use `listUnclaimedDeposits` and filter by the `isMature` field:
...
The `amount` on `ReceivePaymentMethod.CrossChain` is in the source asset's base units, per the route's `CrossChainRoutePair.decimals`. USD-stable sources sit at USD parity, so `1_000_000` is 1 USDC (6 decimals), about $1.
...
The provider's fee in `serviceFeeAmount` is in its own asset, `serviceFeeAsset`, which can differ from both the deposit and the destination asset. Format it with `serviceFeeAssetDecimals`, not the route's decimals.
...
| Event                 | Description                                                                                                                              | UX Suggestion                                                                                               |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| **NewDeposits**       | New deposits were detected. Each deposit includes a `isMature` field indicating whether it has reached the standard claim depth. | Show the deposit to the user as pending until it is claimed.          |
...
| **PaymentSucceeded** | The inbound Spark transfer is claimed and the payment is complete.                                   | Show the payment as complete and call `getInfo` to read the updated balance. The SDK refreshes the cached balance before emitting this event. See [fetching the balance](/llms/kotlin/guide/get_info.md). |
| **PaymentMetadataUpdated** | The provider confirmed the order and the payment now carries its `conversionInfo`. This can arrive after **PaymentSucceeded**, which may report the payment as a plain Spark transfer. | Refresh the payment in the UI with the details carried by the event. |
