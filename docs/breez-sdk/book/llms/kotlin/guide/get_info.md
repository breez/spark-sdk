...
2. Subscribe to events and call `getInfo` again on each `SdkEvent.Synced` event to fetch the latest balance. See [Listening to events](/llms/kotlin/guide/events.md).

| Event | Description | UX Suggestion |
| ----- | ----------- | ------------- |
| `SdkEvent.Synced` | The SDK has synced with the network in the background. | Call `getInfo` to refresh the displayed balance, and refresh the payments list. See [listing payments](/llms/kotlin/guide/list_payments.md). |
