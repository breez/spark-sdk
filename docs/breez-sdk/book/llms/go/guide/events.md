...
| `SdkEventPaymentMetadataUpdated` | `Payment` | Details of a payment already reported changed, for example the conversion info of a cross-chain receive that arrived after the payment settled. |
| `SdkEventNewDeposits` | `DepositInfo` list | On-chain deposits were detected, possibly while still unconfirmed. Show them as pending until claimed: some may be credited early through an [instant or expedited claim](onchain_claims.md#instant-expedited-claims). |
