...
The `SdkEventSynced` event pattern documented in [Listening to events](events.md) is **not available** in this configuration: the SDK has no background subscriber to emit it. Treat `SyncWallet` as the synchronous primitive instead.
