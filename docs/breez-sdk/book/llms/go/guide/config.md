...
USDC/USDT (send and receive) requires explicit opt-in: `DefaultConfig` leaves `CrossChainConfig` unset. Set it to a default `CrossChainConfig` to enable the feature, or to your own to override the slippage default. The SDK only returns routes whose destination is USDC or USDT on a supported chain.
