...
|---------|-------|----------|
| `prepareUnilateralExit` returns no `leaves` | Under `ExitLeafSelection.auto`, no leaf is worth exiting at the current rate, or the leaves' exits already finished | Lower `feeRateSatPerVbyte` or wait for cheaper on-chain fees. A finished exit has nothing left to recover (this is not an error) |
