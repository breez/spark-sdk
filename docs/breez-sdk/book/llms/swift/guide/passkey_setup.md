...
Without the Associated Domains entitlement declared in Xcode, passkey operations on iOS / macOS fail with a configuration error, even when `PasskeyClient.checkAvailability` returns `PasskeyAvailability.available`.
