...
async def prepare_pay(client: BreezClient):
    ...
    try:
        parsed_input = await client.parse(lnurl_pay_url)
        if isinstance(parsed_input, InputType.LIGHTNING_ADDRESS):
            ...
            prepare_response = await client.prepare_lnurl_pay(request=request)
...
async def pay(client: BreezClient, prepare_response: PrepareLnurlPayResponse):
    # ANCHOR: lnurl-pay
    try:
        optional_idempotency_key = "<idempotency key uuid>"
        response = await client.lnurl_pay(
            LnurlPayRequest(
...
async def prepare_pay_fees_included(client: BreezClient, pay_request: LnurlPayRequestDetails):
