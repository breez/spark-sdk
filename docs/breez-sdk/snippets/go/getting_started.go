...
func InitSdkPostgres() (*breez_sdk_spark.BreezSdk, error) {
	...
	maxPoolSize := uint32(8)
	waitTimeoutSecs := uint64(30)
	postgresConfig := breez_sdk_spark.PostgresStorageConfig{
		ConnectionString:   "host=localhost user=postgres dbname=spark",
		// Optional pool settings (all default to nil):
		MaxPoolSize:        &maxPoolSize,       // Max connections in pool
		WaitTimeoutSecs:    &waitTimeoutSecs,   // Timeout waiting for connection
