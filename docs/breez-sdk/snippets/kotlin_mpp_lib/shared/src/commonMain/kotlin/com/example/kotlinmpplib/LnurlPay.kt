...
class LnurlPay {
    suspend fun prepareLnurlPay(client: BreezClient) {
        ...
        try {
            val inputType = client.parse(lnurlPayUrl)
            if (inputType is InputType.LightningAddress) {
                ...
                val prepareResponse = client.prepareLnurlPay(req)
    ...
    suspend fun lnurlPay(client: BreezClient, prepareResponse: PrepareLnurlPayResponse) {
        // ANCHOR: lnurl-pay
        try {
            val optionalIdempotencyKey = "<idempotency key uuid>"
            val response = client.lnurlPay(LnurlPayRequest(prepareResponse, optionalIdempotencyKey))
        } catch (e: Exception) {
            // handle error
        }
    ...
    suspend fun prepareLnurlPayFeesIncluded(client: BreezClient, payRequest: LnurlPayRequestDetails) {
