...
const elements = {
  ...
  lnurlConfirmCommentSection: document.getElementById(
    "lnurl-confirm-comment-section"
  ),
...
function showPaymentNotification(message, type = "success") {
  const color =
    type === "success" ? "#44ff44" : type === "error" ? "#ff4444" : "#646cff";
...
function displayPayments(payments) {
  ...
  if (payments.length === 0) {
    elements.paymentsList.innerHTML =
      '<div style="padding: 1rem; text-align: center; color: #888;">No payments found</div>';
    return;
  }
  ...
  try {
    ...
    elements.prepareLnurlBtn.onclick = async () => {
      ...
      if (!amountSats || amountSats < minSendable || amountSats > maxSendable) {
        alert(
          `Please enter a valid amount between ${minSendable} and ${maxSendable} satoshis`
        );
        return;
      }
...
elements.cancelSendBtn.addEventListener("click", () =>
  hideModal(elements.sendModal)
);
...
elements.cancelLnurlBtn.addEventListener("click", () =>
  hideModal(elements.lnurlPayModal)
);
