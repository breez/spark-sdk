...
pub trait LnurlAuthSigner {
    /// Derives a public key at the given derivation path.
    /// This is used to derive the linking key for LNURL-auth as per LUD-05.
...
pub async fn perform_lnurl_auth<C: RestClient + ?Sized, S: LnurlAuthSigner>(
    ...
    let sig = signer
        ...
        .await?;
    let linking_key = signer.derive_public_key(&derivation_path).await?;
    ...
    callback_url
        .query_pairs_mut()
        .append_pair("key", &linking_key.to_string());
