...
/// The WASM bridge hands both stores the wallet's data *directory*, so this
/// factory has to resolve the file itself. Passing the directory straight to
/// SQLite fails with "unable to open database file".
#[wasm_bindgen_test]
async fn test_default_tree_store_takes_a_directory() {
    let data_dir = "/tmp/breez-sdk-node-tree-store-default-dir-test";
    js_sys::eval(&format!(
        "require('fs').rmSync('{data_dir}', {{ recursive: true, force: true }})"
    ))
    .expect("Failed to clear the test data directory");

    let tree_store_js = create_default_tree_store(data_dir, None)
        .await
        .expect("Failed to create default tree store from a data directory");

    // Lands in the main storage's file: the two share one database.
    let stat = js_sys::eval(&format!(
        "require('fs').existsSync('{data_dir}/storage.sql')"
    ))
    .expect("Failed to stat the database file");
    assert!(
        stat.as_bool().unwrap_or_default(),
        "default tree store did not open the wallet's storage.sql"
    );

    let store = WasmTreeStore::new(tree_store_js);
