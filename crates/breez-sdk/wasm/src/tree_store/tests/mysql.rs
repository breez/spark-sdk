...
// Import the mysql-tree-store package
...
async fn create_test_tree_store(test_name: &str) -> WasmTreeStore {
    ...
    let tree_store_js = create_mysql_tree_store(config, &TEST_IDENTITY, None)
        .await
        .expect("Failed to create mysql tree store instance");
    WasmTreeStore::new(tree_store_js)
}

#[wasm_bindgen_test]
async fn test_new() {
    let store = create_test_tree_store("mysql_tree_new").await;
...
async fn test_incomplete_pedigree_still_spendable() {
    let store = create_test_tree_store("mysql_tree_incomplete_pedigree").await;
...
async fn test_exit_chain_after_swap_update() {
    let store = create_test_tree_store("mysql_tree_swap_update").await;
...
async fn test_store_ancestors_backfills_chain() {
    let store = create_test_tree_store("mysql_tree_store_anc_backfill").await;
    breez_sdk_spark::tree_store_tests::test_store_ancestors_backfills_chain(&store).await;
}

#[wasm_bindgen_test]
async fn test_stored_chain_survives_every_refresh() {
    let store = create_test_tree_store("my_tree_store_chain_survives").await;
    breez_sdk_spark::tree_store_tests::test_stored_chain_survives_every_refresh(&store).await;
}

#[wasm_bindgen_test]
async fn test_store_ancestors_does_not_revive_spent_leaf() {
    let store = create_test_tree_store("mysql_tree_store_anc_no_revive").await;
...
async fn test_store_ancestors_for_absent_leaf() {
    let store = create_test_tree_store("mysql_tree_store_anc_absent").await;
...
async fn test_empty_pedigree_keeps_stored_chain() {
    let store = create_test_tree_store("mysql_tree_empty_pedigree").await;
...
async fn test_stored_chain_replaces_previous() {
    let store = create_test_tree_store("mysql_tree_chain_replace").await;
...
async fn test_add_leaves_duplicate_ids() {
    let store = create_test_tree_store("mysql_tree_add_leaves_dup").await;
    breez_sdk_spark::tree_store_tests::test_add_leaves_duplicate_ids(&store).await;
}

#[wasm_bindgen_test]
async fn test_set_leaves() {
    let store = create_test_tree_store("mysql_tree_set_leaves").await;
...
async fn test_set_leaves_preserves_reservations_for_in_flight_swaps() {
    let store = create_test_tree_store("mysql_tree_set_leaves_swaps").await;
...
async fn test_cancel_reservation_drops_all_when_keep_empty() {
    let store = create_test_tree_store("mysql_tree_cancel_drop_all").await;
...
async fn test_cancel_reservation_nonexistent() {
    let store = create_test_tree_store("mysql_tree_cancel_nonexist").await;
...
async fn test_cancel_reservation_nonexistent_keeps_leaves() {
    let store = create_test_tree_store("mysql_tree_cancel_nonexist_keeps").await;
...
async fn test_payment_reservation_excluded_from_balance() {
    let store = create_test_tree_store("mysql_tree_pay_balance").await;
...
async fn test_try_reserve_success() {
    let store = create_test_tree_store("mysql_tree_try_reserve_ok").await;
    breez_sdk_spark::tree_store_tests::test_try_reserve_success(&store).await;
}

#[wasm_bindgen_test]
async fn test_try_reserve_insufficient_funds() {
    let store = create_test_tree_store("mysql_tree_try_reserve_insuff").await;
...
async fn test_try_reserve_wait_for_pending() {
    let store = create_test_tree_store("mysql_tree_try_reserve_wait").await;
...
async fn test_try_reserve_fail_immediately_when_insufficient() {
    let store = create_test_tree_store("mysql_tree_try_reserve_fail").await;
...
async fn test_try_reserve_min_amount_with_leaves_above_individual_target() {
    let store = create_test_tree_store("mysql_tree_min_above_target").await;
...
async fn test_try_reserve_min_amount_exact_denominations_above_individual() {
    let store = create_test_tree_store("mysql_tree_min_exact_denoms").await;
...
async fn test_notification_after_swap_with_exact_amount() {
    let store = create_test_tree_store("mysql_tree_notif_swap").await;
...
async fn test_notification_on_pending_balance_change() {
    let store = create_test_tree_store("mysql_tree_notif_pending").await;
...
async fn test_spent_leaves_not_restored_by_set_leaves() {
    let store = create_test_tree_store("mysql_tree_spent_not_restored").await;
...
async fn test_spent_ids_cleaned_up_when_no_longer_in_refresh() {
    let store = create_test_tree_store("mysql_tree_spent_cleanup").await;
...
async fn test_add_leaves_not_deleted_by_set_leaves() {
    let store = create_test_tree_store("mysql_tree_add_not_deleted").await;
...
async fn test_old_leaves_deleted_by_set_leaves() {
    let store = create_test_tree_store("mysql_tree_old_deleted").await;
...
async fn test_change_leaves_from_swap_protected() {
    let store = create_test_tree_store("mysql_tree_change_protected").await;
...
async fn test_finalize_with_new_leaves_protected() {
    let store = create_test_tree_store("mysql_tree_fin_new_protected").await;
...
async fn test_add_leaves_clears_spent_status() {
    let store = create_test_tree_store("mysql_tree_add_clears_spent").await;
...
async fn test_set_leaves_skipped_during_active_swap() {
    let store = create_test_tree_store("mysql_tree_skip_active_swap").await;
...
async fn test_set_leaves_skipped_after_swap_completes_during_refresh() {
    let store = create_test_tree_store("mysql_tree_skip_swap_refresh").await;
...
async fn test_set_leaves_proceeds_after_swap_when_refresh_starts_later() {
    let store = create_test_tree_store("mysql_tree_proceeds_later").await;
...
async fn test_payment_reservation_does_not_block_set_leaves() {
    let store = create_test_tree_store("mysql_tree_pay_no_block").await;
...
async fn test_update_reservation_basic() {
    let store = create_test_tree_store("mysql_tree_update_basic").await;
    breez_sdk_spark::tree_store_tests::test_update_reservation_basic(&store).await;
}

#[wasm_bindgen_test]
async fn test_update_reservation_nonexistent() {
    let store = create_test_tree_store("mysql_tree_update_nonexist").await;
...
async fn test_update_reservation_clears_pending() {
    let store = create_test_tree_store("mysql_tree_update_clears").await;
...
async fn test_update_reservation_preserves_purpose() {
    let store = create_test_tree_store("mysql_tree_update_purpose").await;
...
async fn test_get_leaves_not_available() {
    let store = create_test_tree_store("mysql_tree_not_available").await;
...
async fn test_missing_operators_replaced_on_set_leaves() {
    let store = create_test_tree_store("mysql_tree_missing_ops_replace").await;
...
async fn test_missing_from_operators_leaf_not_available() {
    let store = create_test_tree_store("mysql_tree_missing_not_available").await;
...
async fn test_reserve_leaves_by_ids_preserves_order() {
    let store = create_test_tree_store("mysql_tree_reserve_by_ids_order").await;
