...
impl TreeStore for WasmTreeStore {
    ...
    async fn reserve_leaves_by_ids(
        ...
        let promise = self
            .tree_store
            .reserve_leaves_by_ids(ids_js, purpose.to_string())
            .map_err(js_error_to_tree_error)?;
        let result = JsFuture::from(promise)
            .await
            .map_err(js_error_to_tree_error)?;
...
export interface TreeStore {
    ...
    reserveLeavesByIds: (leafIds: string[], purpose: string) => Promise<LeavesReservation>;
    now: () => Promise<number>;
...
extern "C" {
    ...
    #[wasm_bindgen(structural, method, js_name = reserveLeavesByIds, catch)]
    pub fn reserve_leaves_by_ids(
        this: &TreeStoreJs,
        leaf_ids: JsValue,
        purpose: String,
    ) -> Result<Promise, JsValue>;
