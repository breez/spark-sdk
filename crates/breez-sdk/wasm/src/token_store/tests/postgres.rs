...
// Import the postgres-token-store package
#[wasm_bindgen(module = "js/postgres-token-store")]
extern "C" {
    #[wasm_bindgen(js_name = "createPostgresTokenStore", catch)]
    async fn create_postgres_token_store(
...
async fn create_test_token_store(test_name: &str) -> WasmTokenStore {
    ...
    let token_store_js = create_postgres_token_store(config, &TEST_IDENTITY, None)
        .await
        .expect("Failed to create postgres token store instance");
...
async fn test_select_token_outputs() {
    let store = create_test_token_store("pg_token_select_outputs").await;
