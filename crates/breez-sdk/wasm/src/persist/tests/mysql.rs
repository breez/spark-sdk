...
async fn test_watched_deposit_addresses() {
    let storage = create_test_storage("my_watched_deposit_addresses").await;
...
async fn test_spark_settled_bolt11_joins_at_read() {
    let storage = create_test_storage("my_settled_bolt11_join").await;

    breez_sdk_spark::storage_tests::test_spark_settled_bolt11_joins_at_read(Box::new(storage))
        .await;
}

#[wasm_bindgen_test]
async fn test_get_payment_by_invoice() {
    let storage = create_test_storage("my_get_payment_by_invoice").await;
...
async fn test_lightning_payment_settled_over_spark() {
    let storage = create_test_storage("my_settled_over_spark").await;
...
async fn test_spark_settled_bolt11_crud() {
    let storage = create_test_storage("my_spark_settled_bolt11_crud").await;
...
async fn test_insert_boltz_conversion_info() {
    let storage = create_test_storage("my_insert_boltz_conversion_info").await;
...
async fn test_update_boltz_status_to_completed() {
    let storage = create_test_storage("my_update_boltz_status_to_completed").await;
