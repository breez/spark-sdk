 ...
 * or, where applicable, the spec default (ctor `userName`, ctor
 * `userDisplayName`).
 *
 ...
 *
 * Note: `userId` is intentionally not a request field. The WebAuthn
 * `user.id` is generated by the provider on every call and returned
 * via `RegisteredCredential.userId`; the SDK does not let hosts
 * supply one because reusing a value across creates on the same
