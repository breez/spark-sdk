 ...
 * The v18 migration adds tx_type column to payment_details_token
 */
function createOldV17Database(dbPath) {
  const db = new Database(dbPath);

  try {
    // Create the exact schema that should exist at v17 (before tx_type migration)
    ...
    const transaction = db.transaction(() => {
      ...
      // Set database version to 17 (before tx_type migration which is v18)
