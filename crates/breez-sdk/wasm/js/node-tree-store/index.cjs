...
class NodeTreeStore {
  ...
  async setLeaves(leaves, missingLeaves, refreshStartedAtMs) {
    try {
      ...
      this.db.transaction(() => {
        ...
        this._deleteAncestorsForLeaves(deletedIds);
      })();
    } catch (error) {
      if (error instanceof TreeStoreError) throw error;
  ...
  /**
   * Cancel a reservation. Its leaves are marked and the row is dropped.
   * `leavesToKeep` are re-inserted into the available pool, which clears the
   * mark; their ancestors are already stored (they stayed while the leaves were
   * reserved).
   * @param {string} id
