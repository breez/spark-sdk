plugins {
    id("com.android.library")
    id("org.jetbrains.kotlin.android") version "1.9.21"
    id("maven-publish")
    kotlin("plugin.serialization") version "1.9.21"
}

repositories {
    mavenCentral()
    google()
}

android {
    namespace = "technology.breez.spark"
    compileSdk = 34

    defaultConfig {
        minSdk = 24
        consumerProguardFiles("consumer-rules.pro")
    }

    kotlinOptions {
        jvmTarget = "1.8"
    }

    buildTypes {
        getByName("release") {
            @Suppress("UnstableApiUsage")
            isMinifyEnabled = false
            proguardFiles(file("proguard-android-optimize.txt"), file("proguard-rules.pro"))
        }
    }

    sourceSets {
        getByName("main") {
            // Share the built-in CredentialManagerPrfProvider source with the
            // breez-sdk-spark-kmp artifact: one canonical Kotlin file lives
            // under ../../shared/android-passkey and is referenced from both
            // gradle builds.
            java.srcDirs(
                "src/main/kotlin",
                "../../shared/android-passkey/src/main/kotlin",
            )
        }
    }

    publishing {
        singleVariant("release") {
            withSourcesJar()
        }
    }
}

dependencies {
    implementation("com.squareup.okio:okio:3.6.0")
    implementation("net.java.dev.jna:jna:5.18.0@aar")
    implementation("org.jetbrains.kotlin:kotlin-stdlib-jdk7")
    implementation("androidx.appcompat:appcompat:1.6.1")
    implementation("org.jetbrains.kotlinx:kotlinx-coroutines-core:1.9.0")
    implementation("org.jetbrains.kotlinx:kotlinx-serialization-json:1.6.3")
    implementation("org.jetbrains.kotlinx:atomicfu:0.23.1")
    // CredentialManagerPrfProvider — built-in passkey PRF provider
    implementation("androidx.credentials:credentials:1.3.0")
    implementation("androidx.credentials:credentials-play-services-auth:1.3.0")
    // Required for Dispatchers.Main on Android
    implementation("org.jetbrains.kotlinx:kotlinx-coroutines-android:1.8.0")
}

val libraryVersion: String by project

publishing {
    repositories {
        maven {
            name = "breezReposilite"
            url = uri("https://mvn.breez.technology/releases")
            credentials(PasswordCredentials::class)
            authentication {
                create<BasicAuthentication>("basic")
            }
        }
        maven {
            name = "breezGitHubPackages"
            url = uri("https://maven.pkg.github.com/breez/spark-sdk")
            credentials {
                username = System.getenv("GITHUB_ACTOR")
                password = System.getenv("GITHUB_TOKEN")
            }
        }
    }
    publications {
        create<MavenPublication>("maven") {
            groupId = "breez_sdk_spark"
            artifactId = "bindings-android"
            version = libraryVersion

            afterEvaluate {
                from(components["release"])
            }

            pom {
                name.set("breez-sdk-spark")
                description.set("The Breez Spark SDK enables mobile developers to integrate Spark into their apps with a very shallow learning curve.")
                url.set("https://breez.technology")
                licenses {
                    license {
                        name.set("MIT")
                        url.set("https://github.com/breez/spark-sdk/blob/main/LICENSE")
                    }
                }
                scm {
                    connection.set("scm:git:github.com/breez/spark-sdk.git")
                    developerConnection.set("scm:git:ssh://github.com/breez/spark-sdk.git")
                    url.set("https://github.com/breez/spark-sdk")
                }
            }
        }
    }
}
