...
func main() {
	...
	var stableBalanceConfig *breez_sdk_spark.StableBalanceConfig
	...
	// Build SDK
	seed := breez_sdk_spark.SeedMnemonic{Mnemonic: mnemonic, Passphrase: nil}
	builder := breez_sdk_spark.NewSdkBuilder(config, seed)
	if *postgresConnectionString != "" {
		builder.WithPostgresStorage(breez_sdk_spark.DefaultPostgresStorageConfig(*postgresConnectionString))
	} else {
...
func runRepl(sdk *breez_sdk_spark.BreezSdk, tokenIssuer *breez_sdk_spark.TokenIssuer, network breez_sdk_spark.Network, persistence *CliPersistence) error {
	...
	allCommands = append(allCommands, ContactsCommandNames...)
	...
	for {
		...
		if cmdName == "issuer" {
			...
			DispatchContactsCommand(cmdArgs, sdk, rl)
