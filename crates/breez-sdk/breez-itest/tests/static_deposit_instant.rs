...
use breez_sdk_spark::{
    ClaimDepositOutcome, ClaimDepositRequest, GetInfoRequest, InstantClaimStatus,
    ListUnclaimedDepositsRequest, MaxFee, ReceivePaymentMethod, ReceivePaymentRequest,
};
use rstest::*;
...
async fn test_instant_static_deposit_claim_local() -> Result<()> {
    ...
    // A matured deposit's claim settles and carries its payment; an instant claim is
    // submitted and credits on a later sync.
    assert!(
        matches!(claim_resp.outcome, ClaimDepositOutcome::Submitted),
        "instant claim must be submitted, not settled or deferred: {:?}",
        claim_resp.outcome
    );
