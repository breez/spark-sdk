...
/// Two SDK instances share a balanced ConnectionManager (`Some(2)`) and
/// successfully exchange a Spark transfer. Verifies that requests dispatched
/// across a multi-connection pool still carry a successful Spark transfer
/// end-to-end.
#[rstest]
#[test_log::test(tokio::test)]
async fn test_balanced_connection_manager_spark_transfer() -> Result<()> {
    let connection_manager = new_connection_manager(Some(2));

    let alice_dir = Builder::new()
        .prefix("breez-sdk-balanced-cm-alice")
        .tempdir()?;
    let bob_dir = Builder::new()
        .prefix("breez-sdk-balanced-cm-bob")
        .tempdir()?;
