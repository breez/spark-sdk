...
/// Test 5: Send payment from Alice to Bob using Lightning with idempotency key and custom expiry_secs
#[rstest]
#[test_log::test(tokio::test)]
async fn test_05_lightning_idempotency_key_with_expiry(
    #[future] alice_sdk: Result<SdkInstance>,
    #[future] bob_sdk: Result<SdkInstance>,
) -> Result<()> {
    info!("=== Starting test_05_lightning_idempotency_key_with_expiry ===");
    ...
    // Custom expiry: 2 hours = 7200 seconds
    let custom_expiry_secs: u32 = 7200;
    ...
    let bob_invoice = bob
        ...
        .receive_payment(ReceivePaymentRequest {
            payment_method: ReceivePaymentMethod::Bolt11Invoice {
                description: "idempotency test with custom expiry".to_string(),
                amount_sats: Some(5),
    ...
    // Parse and verify the expiry is set correctly
    ...
    info!("=== Test test_05_lightning_idempotency_key_with_expiry PASSED ===");
    Ok(())
}
