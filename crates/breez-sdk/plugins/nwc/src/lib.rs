...
impl SdkNwcService {
    ...
    async fn new_ctx(
        ...
        let handler: Box<dyn RelayMessageHandler> =
            Box::new(SdkRelayMessageHandler::new(Arc::clone(&services)));
    ...
    async fn get_or_create_keypair(&self, persister: &Persister) -> Result<Keys> {
        let get_secret_key = async || -> Result<String> {
            ...
            if let Ok(Some(key)) = persister.get_nwc_seckey().await {
                return Ok(key);
            }

            // If none exists, generate a new one
            let key = nostr_sdk::key::SecretKey::generate().to_secret_hex();
            persister.set_nwc_seckey(key.clone()).await?;
            Ok(key)
        };
...
impl NwcService for SdkNwcService {
    async fn add_connection_string(&self, name: String) -> NwcResult<String> {
        ...
        ctx.persister
            .set_nwc_uri(name.clone(), uri.to_string())
            .await?;
    ...
    async fn list_connection_strings(&self) -> NwcResult<HashMap<String, String>> {
        self.runtime_ctx().await?.persister.list_nwc_uris().await
    }

    async fn remove_connection_string(&self, name: String) -> NwcResult<()> {
        let ctx = self.runtime_ctx().await?;
        ctx.persister.remove_nwc_uri(name).await?;
        ctx.trigger_resubscription().await;
        Ok(())
    }
...
impl Plugin for SdkNwcService {
    ...
    async fn on_start(&self, services: Arc<SdkServices>, storage: Arc<PluginStorage>) {
        ...
        let ctx = match self.new_ctx(Arc::clone(&services), storage, resub_tx).await {
            Ok(ctx) => Arc::new(ctx),
            Err(err) => {
                ...
                loop {
                    tokio::select! {
                        ...
                        Some(_) = resub_rx.recv() => {
                            ...
                            thread_ctx.services.remove_event_listener(&sdk_listener_id).await;
                            break;
                        }
                    }
                }
