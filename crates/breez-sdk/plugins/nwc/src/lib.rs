use std::{collections::HashMap, str::FromStr as _, sync::Weak};
...
#[macros::async_trait]
impl RustPlugin for SdkNwcService {
    fn id(&self) -> String {
                    ...
                    tokio::select! {
                        ...
                        Some(_) = resub_rx.recv() => {
                            ...
                            if let Some(sdk) = sdk.upgrade() {
                                if !sdk.remove_event_listener(&sdk_listener_id).await {
                                    warn!("Could not remove payment event listener");
                                }
                            }
                            break;
                        }
