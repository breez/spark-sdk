...
impl crate::repository::LnurlRepository for LnurlRepository {
    ...
    async fn get_user_by_name(
        ...
        .map(|row| {
            Ok::<_, sqlx::Error>(User {
                ...
                lnurl_private_mode_enabled: row.try_get::<i32, _>(3)? != 0,
            })
        })
        .transpose()?;
        Ok(maybe_user)
    ...
    async fn upsert_user(&self, user: &User) -> Result<(), LnurlRepositoryError> {
        sqlx::query(
            "REPLACE INTO users (domain, pubkey, name, description, lnurl_private_mode_enabled, updated_at)
            VALUES ($1, $2, $3, $4, $5, $6)",
        )
        .bind(&user.domain)
        .bind(&user.pubkey)
        ...
        .bind(i32::from(user.lnurl_private_mode_enabled))
        .bind(now())
        .execute(&self.pool)
        .await?;
        Ok(())
    ...
    async fn get_zap_monitored_users(&self) -> Result<Vec<String>, LnurlRepositoryError> {
        ...
        let rows = sqlx::query(
            "SELECT DISTINCT user_pubkey
             FROM zaps
             WHERE invoice_expiry > $1 AND zap_event IS NULL AND is_user_nostr_key = 0",
        )
        .bind(now)
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn is_zap_monitored_user(&self, user_pubkey: &str) -> Result<bool, LnurlRepositoryError> {
        ...
        let count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*)
             FROM zaps
             WHERE user_pubkey = $1 AND invoice_expiry > $2 AND zap_event IS NULL AND is_user_nostr_key = 0",
        )
        .bind(user_pubkey)
        .bind(now)
        .fetch_one(&self.pool)
    ...
    async fn get_invoice_monitored_users(&self) -> Result<Vec<String>, LnurlRepositoryError> {
        ...
        let rows = sqlx::query(
            "SELECT DISTINCT i.user_pubkey
             ...
             WHERE i.invoice_expiry > $1 AND i.preimage IS NULL AND u.lnurl_private_mode_enabled = 0",
        )
        .bind(now)
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn is_invoice_monitored_user(
        ...
        let count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*)
             FROM invoices i
             JOIN users u ON i.user_pubkey = u.pubkey
             WHERE i.user_pubkey = $1 AND i.invoice_expiry > $2 AND i.preimage IS NULL AND u.lnurl_private_mode_enabled = 0",
        )
        .bind(user_pubkey)
        .bind(now)
        .fetch_one(&self.pool)
