...
use breez_sdk_spark::{
    BreezSdk, GetInfoRequest, Network, PaymentRequest, PrepareSendPaymentRequest,
...
async fn execute_single_payment(sender: &BreezSdk, payment_type: &PaymentType) -> Result<()> {
    match payment_type {
        PaymentType::Transfer { address, amount } => {
            let prepare = sender
                .prepare_send_payment(PrepareSendPaymentRequest {
                    payment_request: PaymentRequest::Raw(address.clone()),
        ...
        PaymentType::Lightning { invoice, .. } => {
            let prepare = sender
                .prepare_send_payment(PrepareSendPaymentRequest {
                    payment_request: PaymentRequest::Raw(invoice.clone()),
                    amount: None,
                    token_identifier: None,
                    conversion_options: None,
                    fee_policy: None,
