...
fn generate_edge_case_payments(config: &ScenarioConfig) -> Vec<PaymentSpec> {
    let mut edge_amounts: Vec<u64> = vec![config.min_amount, config.max_amount];

    if config.min_amount < config.max_amount {
        edge_amounts.push(config.min_amount + 1);
        edge_amounts.push(config.max_amount - 1);

        let range = config.max_amount - config.min_amount;

        edge_amounts.push(config.min_amount + range / 4);
        edge_amounts.push(config.min_amount + range / 2);
        edge_amounts.push(config.min_amount + range * 3 / 4);
    }

    // Powers of two within range
    let mut power_of_2 = 1u64;
    while power_of_2 <= config.max_amount {
        if power_of_2 >= config.min_amount {
            edge_amounts.push(power_of_2);
        }
        power_of_2 *= 2;
    }

    // Sort + deduplicate
    edge_amounts.sort_unstable();
    edge_amounts.dedup();
