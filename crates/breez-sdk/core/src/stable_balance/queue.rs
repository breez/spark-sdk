...
//!
//! Serializes all conversion tasks (per-receive and auto-convert) through a single
//! queue to eliminate race conditions between the two paths.
...
struct ConversionQueueState {
    ...
    /// Whether an auto-convert task is pending.
...
impl ConversionQueue {
    ...
    /// Per-receive tasks take priority over auto-convert.
    ...
    pub async fn next_task(&self) -> Option<ConversionTask> {
        ...
        } else if state.auto_convert_pending && state.per_receive.is_empty() {
            // Only run auto-convert when no per-receive tasks exist (including deferred).
            // Deferred tasks may still be resolved by a PaymentSucceeded event and need those sats.
...
impl StableBalance {
    ...
    pub(super) fn spawn_conversion_worker(&self, mut shutdown_receiver: watch::Receiver<()>) {
        ...
        tokio::spawn(
            async move {
                ...
                // Restart recovery: re-queue any per-receive conversions from a previous session
    ...
    ///
    /// Loads persisted pending conversions (with their state) and restores them
    /// directly into the queue state.
