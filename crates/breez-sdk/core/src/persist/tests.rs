...
pub async fn test_insert_payment_metadata_returns_false_for_noop(storage: Box<dyn Storage>) {
    ...
    let changed = storage
        .insert_payment_metadata(payment_id.clone(), metadata_idempotent)
        .await
        .unwrap();
    assert!(
        !changed,
    ...
    // An already-set column changing value is the common transition a
    // conversion goes through, and must report changed on every backend.
    let metadata_advanced = PaymentMetadata {
        conversion_status: Some(crate::ConversionStatus::Completed),
        ..Default::default()
    };
    let changed = storage
        .insert_payment_metadata(payment_id.clone(), metadata_advanced.clone())
        .await
        .unwrap();
    assert!(
        changed,
        "advancing an already-set column should report row changed"
    );

    let changed = storage
        .insert_payment_metadata(payment_id.clone(), metadata_advanced)
        .await
        .unwrap();
    assert!(
        !changed,
        "idempotent replay of the advanced column should report no change"
    );

    // A changed field inside an already-set JSON column. Backends compare this
    // differently (semantic JSON vs serialized text), so pin it explicitly.
    let metadata_json_changed = PaymentMetadata {
        conversion_info: Some(crate::ConversionInfo::Amm {
            pool_id: "pool_noop".to_string(),
            conversion_id: "conv_noop".to_string(),
            status: crate::ConversionStatus::Completed,
    ...
    let changed = storage
        .insert_payment_metadata(payment_id.clone(), metadata_json_changed.clone())
        .await
        .unwrap();
    assert!(
        changed,
        "a changed field inside an existing JSON column should report row changed"
    );

    let changed = storage
        .insert_payment_metadata(payment_id, metadata_json_changed)
        .await
        .unwrap();
    assert!(
        !changed,
        "idempotent replay of the changed JSON column should report no change"
    );
}
