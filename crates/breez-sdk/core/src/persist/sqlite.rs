...
mod tests {
    ...
    async fn test_conversion_refund_needed_filtering() {
        let temp_dir =
            tempdir::TempDir::new("sqlite_storage_conversion_refund_needed_filtering").unwrap();
        let storage = SqliteStorage::new(temp_dir.path()).unwrap();
    ...
    async fn test_token_transaction_type_filtering() {
        let temp_dir =
            tempdir::TempDir::new("sqlite_storage_token_transaction_type_filter").unwrap();
        let storage = SqliteStorage::new(temp_dir.path()).unwrap();
    ...
    async fn test_migration_tx_type() {
        ...
        let temp_dir = tempdir::TempDir::new("sqlite_migration_tx_type").unwrap();
        let db_path = temp_dir.path().join(super::DEFAULT_DB_FILENAME);

        // Step 1: Create database at version 20 (before tx_type migration)
        {
            let mut conn = Connection::open(&db_path).unwrap();
            let migrations_before_tx_type: Vec<_> = SqliteStorage::current_migrations()
                .iter()
                .take(21) // Migrations 0-20 (index 21 is the tx_type migration)
                .map(|s| M::up(s))
                .collect();
