...
use spark_wallet::{
    ...
    LightningReceivePayment, Preimage,
};
...
use crate::{
    ...
    utils::payments::record_spark_settled_bolt11_receive,
};
...
pub(super) async fn receive_bolt11_invoice_inner(
    ...
    if matches!(fallback, LightningReceiveFallback::Invoice(_)) {
        ...
        match sdk.spark_wallet.extract_spark_fallback(&receive.invoice) {
            Ok(Some(embedded)) => {
                if let Err(e) = record_spark_settled_bolt11_receive(
                    &sdk.storage,
                    &embedded.encoded,
                    &receive.invoice,
                )
                .await
                {
                    ...
                    // as a plain Spark receive.
                    error!("Failed to record the Spark-settled Bolt11 receive: {e:?}");
                }
            }
...
async fn build_fallback(
    ...
    // receive request takes. Left open it would outlive the Bolt11, and a
    // transfer settling it would be attributed to an invoice already expired.
    let expiry_secs = expiry_secs.unwrap_or(DEFAULT_RECEIVE_EXPIRY_SECS);
