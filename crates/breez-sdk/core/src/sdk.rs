...
use spark_wallet::{
    ExitSpeed, InvoiceDescription, Preimage, SparkAddress, SparkWallet, TokenTransaction,
...
use crate::{
    ...
    ClaimDepositRequest, ClaimDepositResponse, ClaimSparkHtlcRequest, ClaimSparkHtlcResponse,
...
impl BreezSdk {
    ...
    async fn send_spark_address(
        ...
        let payment = if let Some(identifier) = token_identifier {
            ...
            let transfer = self
                .spark_wallet
                .transfer(amount.try_into()?, &spark_address)
                .await?;
            transfer.try_into()?
        };
