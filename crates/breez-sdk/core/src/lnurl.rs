...
/// Default `LnurlServerClient` implementation using `RestClient` abstraction.
/// Works with both bitreq (native) and reqwest (native with feature or WASM).
...
impl LnurlServerClient for DefaultLnurlServerClient {
    ...
    async fn check_username_available(&self, username: &str) -> Result<bool, LnurlServerError> {
        ...
        let response = self
            .rest_client
            .get_request(url, Some(self.get_headers()))
            .await
    ...
    async fn recover_lightning_address(
        ...
        let response = self
            .rest_client
            .post_request(url, Some(self.get_headers()), Some(body))
            .await
            .map_err(|e| LnurlServerError::RequestFailure(e.to_string()))?;
    ...
    async fn unregister_lightning_address(
        ...
        let response = self
            .rest_client
            .delete_request(url, Some(self.get_headers()), Some(body))
            .await
            .map_err(|e| LnurlServerError::RequestFailure(e.to_string()))?;
