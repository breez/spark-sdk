...
use breez_sdk_spark::{
    EventListener, Network, SdkBuilder, SdkEvent, Seed, create_postgres_storage,
    create_postgres_storage_config, default_config,
};
use clap::Parser;
...
async fn run_interactive_mode(
    ...
    if let Some(connection_string) = postgres_connection_string {
        let postgres_config = create_postgres_storage_config(connection_string);
