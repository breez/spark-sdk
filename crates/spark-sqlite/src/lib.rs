...
//! through, carrying no pool metadata. Each ancestor row is owned by the leaf
//! whose chain it belongs to (its primary key is `(leaf_id, id)`), so a node
//! shared by several leaves' chains is stored once per leaf. A leaf that splits
//! is deleted from the leaf table, and its children carry it as an ancestor.
//! Every deletion of a leaf row deletes that leaf's own ancestor rows in the
//! same transaction, so no ancestor row can ever outlive its leaf. Only runtime
...
const TREE_MIGRATIONS: &[&str] = &[
    ...
    // v2: keep a leaf no operator reports rather than removing it, so the chain
...
impl SqliteTreeStore {
    ...
    fn upsert_ancestors(
        ...
        .map_err(|e| generic("delete stale ancestors", e))?;
        for node in nodes {
            ...
            conn.execute(
                "INSERT INTO brz_tree_ancestors
                 ...
                 ON CONFLICT(leaf_id, id) DO UPDATE SET
    ...
    /// Deletes the ancestor rows owned by each of `leaf_ids`.
    fn delete_ancestors_for_leaves(
        conn: &Connection,
        leaf_ids: &[String],
    ) -> Result<(), TreeServiceError> {
    ...
    fn delete_reserved(conn: &Connection, reservation_id: &str) -> Result<(), TreeServiceError> {
        conn.execute(
...
impl TreeStore for SqliteTreeStore {
    ...
    async fn finalize_reservation(
        ...
        Self::delete_reserved(&tx, id)?;
        Self::delete_ancestors_for_leaves(&tx, &reserved)?;
        tx.execute(
            "DELETE FROM brz_tree_reservations WHERE id = ?1",
            params![id],
        )
...
mod tests {
    ...
    shared_tests!(
        ...
        test_stored_chain_survives_every_refresh,
