...
impl ExitChainResolver {
    ...
    async fn resolve_batch(
        ...
        // exitable and build a set that cannot confirm. A partial one is also
        // nothing to announce: it leaves the leaf just as un-exitable as before,
        // and comes back byte-identical on every retry until the operators fill
        // it in.
...
mod tests {
    ...
    impl TreeService for MockTreeService {
        ...
        async fn retire_leaves(&self, _leaf_ids: &[TreeNodeId]) -> Result<(), TreeServiceError> {
            unimplemented!("not exercised by ExitChainResolver")
        }
    ...
    #[async_test_all]
    async fn test_an_unanswered_leaf_stores_nothing() {
        let leaf = create_test_node_with_parent("leaf", Some("root"), TreeNodeStatus::Available);
