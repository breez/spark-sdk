...
use crate::{
    ...
    signer::{PrepareTokenTransactionRequest, Signer, SparkSigner, TokenTransactionKind},
    token::{
...
impl TokenService {
    ...
    async fn transfer_tokens_inner(
        ...
        let result = spark_token_primitives::construct_partial_transfer_transaction(
            spark_token_primitives::TransferBuildRequest {
                ...
                network: u32::try_from(self.network.to_proto_network() as i32).map_err(|_| {
    ...
    async fn start_transaction(
        ...
        let signature = self
            .sign_token_digest(TokenTransactionKind::Partial, &partial_tx_hash)
            .await?;
    ...
    async fn create_per_operator_signatures(
        ...
        for operator in self.operator_pool.get_all_operators() {
            ...
            let signature = self
                .sign_token_digest(TokenTransactionKind::Final, &final_hash)
                .await?;
