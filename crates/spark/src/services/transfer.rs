...
use crate::operator::rpc::spark::{
    ExtendLeafRequest, FinalizeNodeSignaturesRequest, NodeSignatures, RefreshTimelockRequest,
    SigningJob, TransferFilter, TreeNodeIds,
};
use crate::operator::rpc::{self as operator_rpc, OperatorRpcError, QueryAllNodesRequest};
...
impl<S: Signer> TransferService<S> {
    ...
    pub async fn transfer_leaves_to(
        ...
        let leaves = self.check_timelock_nodes(leaves).await?;
    ...
    pub async fn claim_transfer(
        ...
        loop {
            ...
            return Box::pin(self.check_timelock_nodes(result)).await;
        }
    }
