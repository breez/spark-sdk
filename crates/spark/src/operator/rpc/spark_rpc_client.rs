...
impl SparkRpcClient {
    ...
    pub async fn query_utxos_for_identity(
        &self,
        req: QueryUtxosForIdentityRequest,
    ) -> Result<QueryUtxosForIdentityResponse> {
        debug!("Calling query_utxos_for_identity with request: {:?}", req);
        Ok(self
            .spark_service_client()
            .await?
            .query_utxos_for_identity(req)
            .await?
            .into_inner())
    }
