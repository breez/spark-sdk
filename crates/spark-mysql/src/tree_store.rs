...
impl MysqlTreeStore {
    ...
    /// Creates a new `MysqlTreeStore` from an existing connection pool and
    /// foreign-key mode.
    ///
    /// `identity` is the 33-byte secp256k1 pubkey of the tenant.
...
/// Creates a `MysqlTreeStore` instance from an existing connection pool and
/// foreign-key mode.
///
/// `identity` is the 33-byte secp256k1 pubkey scoping all reads and writes.
pub async fn create_mysql_tree_store_from_pool_with_foreign_key_mode(
    pool: Pool,
    identity: &[u8],
    foreign_key_mode: MysqlForeignKeyMode,
) -> Result<Arc<dyn TreeStore>, MysqlError> {
    Ok(Arc::new(
        MysqlTreeStore::from_pool_with_foreign_key_mode(pool, identity, foreign_key_mode).await?,
    ))
}
