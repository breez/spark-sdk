...
impl MysqlTokenStore {
    ...
    pub async fn from_config(
        ...
        Self::init(pool, identity, config.foreign_key_mode).await
    }

    /// `identity` is the 33-byte secp256k1 pubkey of the tenant.
    pub async fn from_pool(pool: Pool, identity: &[u8]) -> Result<Self, MysqlError> {
        Self::from_pool_with_foreign_key_mode(pool, identity, MysqlForeignKeyMode::default()).await
    }

    /// Creates a new `MysqlTokenStore` from an existing connection pool and
    /// foreign-key mode.
    ///
    /// `identity` is the 33-byte secp256k1 pubkey of the tenant.
    pub async fn from_pool_with_foreign_key_mode(
        pool: Pool,
        identity: &[u8],
        foreign_key_mode: MysqlForeignKeyMode,
    ) -> Result<Self, MysqlError> {
        Self::init(pool, identity, foreign_key_mode).await
    }
...
fn token_outputs_create_table_sql(foreign_key_mode: MysqlForeignKeyMode) -> &'static str {
...
/// Creates a `MysqlTokenStore` from an existing connection pool and
/// foreign-key mode.
///
/// `identity` is the 33-byte secp256k1 pubkey scoping all reads and writes.
pub async fn create_mysql_token_store_from_pool_with_foreign_key_mode(
    pool: Pool,
    identity: &[u8],
    foreign_key_mode: MysqlForeignKeyMode,
) -> Result<Arc<dyn TokenOutputStore>, MysqlError> {
    Ok(Arc::new(
        MysqlTokenStore::from_pool_with_foreign_key_mode(pool, identity, foreign_key_mode).await?,
    ))
}
