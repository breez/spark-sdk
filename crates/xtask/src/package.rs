...
fn package_wasm_target(
    ...
    // For Node.js target, copy the JavaScript storage implementations.
    // Postgres no longer needs JS files copied: the wasm bundle calls
    // the Rust `PostgresBackend` directly through `pg-wasm`, and the
    // bridge's pg-wasm-bridge.cjs is already pulled in via the
    // wasm-bindgen snippets mechanism.
    if target == "nodejs" {
